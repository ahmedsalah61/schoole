...
class Add_homework_cubit extends Cubit<Add_Homework_states> {
  ...
  Future pick_file() async {
    homweork_file = await FilePicker.platform.pickFiles(allowMultiple: false);
  ...
  Future get_teacher_classes() async {
    ...
    MockDioHelper.getData(url: 'getSafs', token: token)
        .then((value) {
          classes = value as List;
  ...
  Future get_teacher_sections({required int subject_id}) async {
  ...
  Future get_teacher_subject() async {
    ...
    MockDioHelper.getData(url: 'getSubjects', token: token)
        .then((value) {
          subject = value as List;
          print(value);
          ...
          data: FormData.fromMap({
            ...
            if (homefile != null)
              'file': MultipartFile.fromBytes(
                homefile!,
                filename: homweork_file!.files.single.name,
              ),
          }),
        )
        .then((value) {
