...
class Chat_cubit extends Cubit<chat_states>{
  ...
  Future sendmessage({
    ...
    return await MockDioHelper.postData(url: 'sendMessage',
        token:token,
        data: {
          'message':message,
  ...
  Future getMessages({
    ...
    await MockDioHelper.getData(url: 'getMessage',
        token: token,
        // query: {
        //   'id':id
        // }
      ...
      CacheHelper.saveData(key: 'room_id', value: value['data']['profile']['room_id']);
      //print(messagesList);
